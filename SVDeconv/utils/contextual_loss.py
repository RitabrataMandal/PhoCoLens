...
def _random_pooling(feats, output_1d_size=100):
    ...
    feats_sample, indices = _random_sampling(feats[0], output_1d_size ** 2, None)
    feats_sample = feats_sample.to(device)
    indices = indices.to(device)

    res = [feats_sample]

    for i in range(1, len(feats)):
        feats_sample, _ = _random_sampling(feats[i], -1, indices)
        feats_sample = feats_sample.to(device)
        res.append(feats_sample)

    res = [
        feats_sample.view(N, C, output_1d_size, output_1d_size) for feats_sample in res
    ]
...
def _centered_by_T(I, T):
    mean_T = (
        T.mean(dim=0, keepdim=True).mean(dim=2, keepdim=True).mean(dim=3, keepdim=True)
    )
...
def _create_using_dotP(I_features, T_features):
    I_features, T_features = _centered_by_T(I_features, T_features)
    I_features = _normalized_L2_channelwise(I_features)
    T_features = _normalized_L2_channelwise(T_features)
...
def calculate_CX_Loss(I_features, T_features, distance="cosine", b=1.0, h=0.1, max_1d_size=64):
    assert I_features.size() == T_features.size(), "Features must be of same shape"

    N, C, H, W = I_features.size()

    if H * W > max_1d_size ** 2:
        I_features = _random_pooling(I_features, output_1d_size=max_1d_size)
        T_features = _random_pooling(T_features, output_1d_size=max_1d_size)
    ...
    if distance == "L1":
        raw_distance = _create_using_L1(I_features, T_features)

    elif distance == "L2":
        raw_distance = _create_using_L2(I_features, T_features)
    else:
        raw_distance = _create_using_dotP(I_features, T_features)

    if torch.sum(torch.isnan(raw_distance)) == torch.numel(raw_distance) or torch.sum(
        torch.isinf(raw_distance)
    ) == torch.numel(raw_distance):
        print(raw_distance)
        raise ValueError("NaN or Inf in raw_distance")

    relative_distance = _calculate_relative_distance(raw_distance)
    if torch.sum(torch.isnan(relative_distance)) == torch.numel(
        relative_distance
    ) or torch.sum(torch.isinf(relative_distance)) == torch.numel(relative_distance):
        print(relative_distance)
        raise ValueError("NaN or Inf in relative_distance")
    del raw_distance

    exp_distance = torch.exp((b - relative_distance) / h)
    if torch.sum(torch.isnan(exp_distance)) == torch.numel(exp_distance) or torch.sum(
        torch.isinf(exp_distance)
    ) == torch.numel(exp_distance):
        print(exp_distance)
...
if __name__ == "__main__":
    from loss import Vgg16FeatureExtractor

    loss_network = Vgg16FeatureExtractor()

    I = torch.rand(1, 3, 128, 128)
    T = torch.randn(1, 3, 128, 128)

    I_feat = loss_network(I)
    T_feat = loss_network(T)

    context_loss = calculate_CX_Loss(I_feat.relu2_2, T_feat.relu2_2)
    percep_loss = F.l1_loss(I_feat.relu2_2, T_feat.relu2_2)
    breakpoint()